...
let wait = (message, callback, delay) => {
	...
	return new Promise((resolve, reject) => {
		...
		handler = setInterval(() => {
			...
			process.stdout.write(`\x1b[7m[WP_Boilerplate]\x1b[27m ${spinner[waitcounter % 12]}  ${message}`);
...
let loadSettings = (reinstall) => {
	...
	if (!sh.test('-f', setupSettingsFilename)) {
		if (reinstall && !sh.test('-f', setupSettingsFilename)) {
			...
			halt('Could not find \'setup.yml\' or \'config/setup.yml\'. Please use the \'wp_boilerplate init <slug>\' command to create a new project folder and \'setup.yml\'.');
		} else {
			halt('Could not find \'setup.yml\'. Please use the \'wp_boilerplate init <slug>\' command to create a new project folder and \'setup.yml\'. If the current project has been set up previously, you can run \'wp_boilerplate setup --reinstall\' and \'config/setup.yml\' will be used to bring the Docker containers back up and reconfigure them.');
		}
	}
	...
	if (sh.exec(`docker ps -aqf name=${settings.slug}_wp`, {silent: true}).stdout) {
		if (reinstall) {
			...
			halt(`There's already a Docker container called '${settings.slug}_wp'. If this container belongs to another project remove all containers for that project or rename this one before running setup. Otherwise run \'wp_boilerplate setup --reinstall\' to re-use already existing Docker containers for this project.`);
		}
	}
...
let installWordPress = (webPort, settings) => {
	...
	install.stdout.on('data', data => {
		...
		if (settings.wp.skip_default_themes) {
			wp(`theme delete "twentytwenty" "twentynineteen" "twentyeighteen" "twentyseventeen" "twentysixteen" "twentyfifteen"`);
		}
		// WordPress options
...
let init = (slug, options) => {
	...
	echo(`Project initial 'setup.yml' file created. Edit settings in this file and run 'wp_boilerplate setup' to setup the project.`);
};
