...
try {
	settings.webPort = exec('docker-compose port web 80')
		.toString()
		.replace(/^.*:(\d+)\n$/g, '$1')
	settings.dbPort = exec('docker-compose port db 3306')
		.toString()
		.replace(/^.*:(\d+)\n$/g, '$1')
} catch (ex) {
...
function clean(done) {
	return del([base.theme], { force: true }).then(function() {
...
function cleanScripts(done) {
	return del(base.theme + dest.scripts).then(function() {
...
function functions(cb) {
	...
	return gulp.src(glob.functions).pipe(
		tap(function(file) {
			fs.appendFileSync(
...
function styles() {
	return gulp
		...
		.pipe(
			postcss(options.postcss).on('error', function(error) {
...
function scripts(done) {
	...
	return gulp
		.src(glob.scriptMain)
		.pipe(
			webpackStream(require('./webpack.config')(environment), webpack).on('error', function(error) {
				this.emit('end')
			})
		)
...
function imports(cb) {
	...
	settings.imports.plugins.forEach(function(plugin) {
		importsPipes.push(
			gulp
				.src(plugin.src, { base: path.dirname(plugin.path) })
...
function wordmove(cb) {
	...
	try {
		const wordmove = yaml.safeLoad(fs.readFileSync('./config/wordmove.yml', 'utf8')) || {}
...
function wpconfig(cb) {
	...
	const dockerCmd = 'docker-compose exec -u www-data -T wp',
		wpPort = exec(dockerCmd + ' wp option get siteurl')
			.toString()
			.replace(/^.*:(\d+)\n$/g, '$1')
	if (wpPort != settings.webPort) {
		...
		exec(
			dockerCmd +
			' bash -c \'wp option update home "http://localhost:' +
			settings.webPort +
...
gulp.task(
	...
	gulp.series(
		...
		gulp.parallel(
			...
			views,
			styles,
			gulp.series(cleanScripts, scripts),
			images,
			fonts,
			imports,
			wordmove
